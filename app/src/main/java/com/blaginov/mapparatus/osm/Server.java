...
import com.blaginov.mapparatus.exception.OsmIOException;
...
public class Server {
	 ...
	 * Location of OSM API
	 */
	private final String SERVER_URL = "http://api06.dev.openstreetmap.org/api/0.6/";
...
//	private final String createdByKey;
	...
	private String generator = "Mapparatus";
	 ...
	 */
	public Server(OAuthConsumer consumer, String accesstoken, String accesstokensecret) {
	...
	public Capabilities getCapabilities() {
		...
		if (capabilities == null) {
			...
			try {
				...
				con.setRequestProperty("User-Agent", "Mapparatus");
				try {
					//connection.getOutputStream().close(); GET doesn't have an outputstream
	...
	public InputStream getStreamForElement(String mode, final String type, final long id) throws OsmServerException, IOException {
		...
		URL url = new URL(SERVER_URL + type + "/" + id + (mode != null ? "/" + mode : ""));
		HttpURLConnection con = (HttpURLConnection) url.openConnection();
		boolean isServerGzipEnabled = false;
		...
		public void run() {
			...
			try {
				...
				Toast.makeText(mainCtx,
					  "toast_download_failed", Toast.LENGTH_LONG).show();
			} catch (Exception ex) {
...
//		elem.addOrUpdateTag(createdByTag, createdByKey);
	...
	private HttpURLConnection openConnectionForWriteAccess(final URL url, final String requestMethod, final String contentType)
		...
		connection.setRequestProperty("User-Agent", "Mapparatus");
		connection.setConnectTimeout(TIMEOUT);
		connection.setReadTimeout(TIMEOUT);
		connection.setRequestMethod(requestMethod);
            Log.i("chechi", accesstoken + "  " + accesstokensecret);
            OAuthConsumer prosumer = new DefaultOAuthConsumer("CNFPvywkyDrqEaA6CiBgdhLxhaRJgRdHdV146pR1", "7XQE30u7qeKQpVp0sT4ZRyfsS5UWoQrOtyUyG7md");
            prosumer.setTokenWithSecret("VPouKzbZAcL3tFxs0dCLw3ih2YMAqTiN8xigwMgE", "cntt3N8dsHjbNxzLDDpKUu6N67s2o9KEHUZAy7o5");
            /*
            CommonsHttpOAuthConsumer prosumer = new CommonsHttpOAuthConsumer("CNFPvywkyDrqEaA6CiBgdhLxhaRJgRdHdV146pR1", "7XQE30u7qeKQpVp0sT4ZRyfsS5UWoQrOtyUyG7md");
            prosumer.setTokenWithSecret("VPouKzbZAcL3tFxs0dCLw3ih2YMAqTiN8xigwMgE", "cntt3N8dsHjbNxzLDDpKUu6N67s2o9KEHUZAy7o5");
			// sign the request*/

			try {
                prosumer.sign(connection);
...
//		elem.addOrUpdateTag(createdByTag, createdByKey);
	...
	public String getBaseURL() {
		return SERVER_URL.replaceAll("/api/[0-9]+(?:\\.[0-9]+)+/?$", "/");
	}
	...
	public Collection<Bug> getNotesForBox(Rect area, long limit) {
		...
		try {
			...
			URL url = new URL(SERVER_URL  + "notes?" +
					"limit=" + limit + "&" +
					"bbox=" +
					area.left / 1E7d +
					"," + area.bottom / 1E7d +
...
//						+ ", response code: " + con.getResponseCode() + " \"" + con.getResponseMessage() + "\"");
		...
		if (bug.getId() != 0) {
			...
			try {
				try {
					...
					connection = 
							openConnectionForWriteAccess(new URL(SERVER_URL  + "notes/"+Long.toString(bug.getId())+"/comment?text="  +URLEncoder.encode(comment.getText(), "UTF-8")), "POST", "text/url");
		...
		if (bug.getId() == 0 && bug.comments.size() == 0) {
			...
			try {
				try {
					// setting text/xml here is a hack to stop signpost (the oAuth library) from trying to sign the body which will fail
					connection = 
							openConnectionForWriteAccess(new URL(SERVER_URL  + "notes?lat=" + (bug.getLat() / 1E7d)+"&lon=" + (bug.getLon() / 1E7d) + "&text=" +URLEncoder.encode(comment.getText(), "UTF-8")), "POST", "text/xml");
					OutputStreamWriter out = new OutputStreamWriter(connection.getOutputStream(), Charset
							.defaultCharset());
					// out.write("text="+URLEncoder.encode(comment.getText(), "UTF-8") + "\r\n");
					out.flush();
		...
		if (bug.getId() != 0) {
			...
			try {
				try {
					// setting text/xml here is a hack to stop signpost (the oAuth library) from trying to sign the body which will fail
					connection = 
							openConnectionForWriteAccess(new URL(SERVER_URL  + "notes/"+Long.toString(bug.getId())+"/close"  ), "POST", "text/xml");
					if (connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
						throw new OsmServerException(connection.getResponseCode(), "The API server does not except the request: " + connection
								+ ", response code: " + connection.getResponseCode() + " \"" + connection.getResponseMessage() + "\"");
					}
	...
	public boolean reopenNote(Bug bug) {
		...
		if (bug.getId() != 0) {
			...
			try {
				try {
					connection = 
							openConnectionForWriteAccess(new URL(SERVER_URL  + "notes/"+Long.toString(bug.getId())+"/reopen"  ), "POST", "text/xml");
					if (connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
						throw new OsmServerException(connection.getResponseCode(), "The API server does not except the request: " + connection
								+ ", response code: " + connection.getResponseCode() + " \"" + connection.getResponseMessage() + "\"");
					}
	...
	public void uploadTrack(Track track, String description, String tags, Visibility visibility) throws MalformedURLException, ProtocolException, IOException, IllegalArgumentException, IllegalStateException, XmlPullParserException {
		...
		try {
			...
			connection = 
					openConnectionForWriteAccess(new URL(SERVER_URL  + "gpx/create"), "POST", "multipart/form-data;boundary="+boundary);
